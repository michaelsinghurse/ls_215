...
function parseDateString(string) {
  let dateParts = string.replace("Date: ", "").split("-");
  let month = Number(dateParts[0]) - 1;  // 0 = January, 11 = December
  let day = Number(dateParts[1]);
  let year = Number(dateParts[2]);
  return new Date(year, month, day);
}

function displayResults(count, minDate, maxDate) {
  console.log(`Count of Email: ${count}`);
  console.log(`Date Range: ${minDate.toDateString()} - ${maxDate.toDateString()}`);
}

function mailCount(data) {
  let emails = cleanData(data).split(EMAIL_DELIMITER);
  let dates = emails.map(email => {
    return parseDateString(email.split(PART_DELIMITER)[DATE_PART_INDEX]);
  });
  dates.sort((date1, date2) => date1 - date2);
  displayResults(emails.length, dates[0], dates[dates.length - 1]);
}
